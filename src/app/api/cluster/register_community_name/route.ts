export async function POST(request: Request) {
  ...
  const authKey = process.env.NEXT_PUBLIC_PLUME_CLUSTER_COMMUNITY_AUTH_KEY;
  if (!apiKey || !authKey) throw new Error('API key is not defined');

  try {
    const response = await fetch(`https://api.clusters.xyz/v1/names/community/plume/register?testnet=true`, {
      method: "POST",
      headers: {
